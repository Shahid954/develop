...
import com.zenwork.pageObject.RequestTinPaymentPage;
import com.zenwork.pageObject.SideNavigation;

public class ValidateTinMatchRequestForSingleRecipient extends TestBase {
	...
	@BeforeClass
	public void setUp() {
		appLibrary = new AppLibrary("ValidateTinMatcInManageRecipientPage");
	}
	...
	public void validateRequestTinMatch() throws Exception {
		appLibrary.getDriverInstance();
		appLibrary.launchApp();// launching the application in browser
		HomePage ap = new HomePage(appLibrary);
		DashboardPage dp = new DashboardPage(appLibrary);
		...
		mr.uploadFile();
		mr.requestTinMatchWithPrepayForSingleRecipient();
